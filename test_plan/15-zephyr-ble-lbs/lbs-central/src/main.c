...
static uint8_t remote_led_last_written = LED_PHYS_OFF_LEVEL;
static uint8_t remote_led_inflight = LED_PHYS_OFF_LEVEL;
...
static struct k_work_delayable discover_start_work;
...
static void discover_start_handler(struct k_work *work);
static void gatt_write_cb(struct bt_conn *conn, uint8_t err,
			  struct bt_gatt_write_params *params);
static void try_send_remote_led_write(void);
...
static void try_send_remote_led_write(void)
{
	...
	if (write_in_progress) {
		LOG_DBG("button: write already in progress, pending target=%u", remote_led_level);
		return;
	}

	if (remote_led_level == remote_led_last_written) {
		LOG_DBG("button: target already synced (%u)", remote_led_level);
		return;
	}

	write_params.handle = onoff_action_handle;
	write_params.offset = 0U;
	remote_led_inflight = remote_led_level;
	write_params.data = &remote_led_inflight;
	write_params.length = sizeof(remote_led_inflight);
	...
	if (err) {
		...
		LOG_INF("button: write started remote led=%u (handle=0x%04x)", remote_led_inflight,
			onoff_action_handle);
	}
}
			  ...
			  struct bt_gatt_write_params *params)
{
	...
	remote_led_last_written = remote_led_inflight;
	LOG_INF("GATT write ok (remote led=%u)", remote_led_inflight);
				...
				struct bt_gatt_discover_params *params)
{
	...
	if (params->type == BT_GATT_DISCOVER_CHARACTERISTIC) {
		...
		if (!onoff_action_found && bt_uuid_cmp(chrc->uuid, BT_UUID_ONOFF_ACTION) == 0) {
			...
			LOG_INF("remote LED control ready; press sw0 to toggle peer LED");
		}
...
static void discover_start_handler(struct k_work *work)
{
	...
	discover_onoff_service(default_conn);
}
...
static void connected(struct bt_conn *conn, uint8_t err)
{
	...
	remote_led_level = LED_PHYS_OFF_LEVEL;
	remote_led_last_written = LED_PHYS_OFF_LEVEL;
	remote_led_inflight = LED_PHYS_OFF_LEVEL;
	write_in_progress = false;
	k_work_reschedule(&discover_start_work, K_MSEC(200));
}
...
static void disconnected(struct bt_conn *conn, uint8_t reason)
{
	...
	k_work_cancel_delayable(&discover_start_work);
...
int main(void)
{
	...
	k_work_init_delayable(&discover_start_work, discover_start_handler);
